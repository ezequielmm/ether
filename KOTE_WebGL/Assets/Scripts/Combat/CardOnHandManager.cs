...
namespace CardManagement
{
    public class CardOnHandManager : MonoBehaviour
    {
        ...
        {
            if (cardId != cardData.id && !cardIsShowingUp)
            {
        ...
        {
            if (cardId != cardData.id && cardActive)
            {
        ...
        private void OnMoveCompleted()
        {
            ...
            if (discardAfterMove)
            {
                ...
                GameManager.Instance.EVENT_CARD_DISABLED.Invoke(cardData.id);
            }
        ...
        private void UpdateCardBasedOnEnergy(int energy)
        {
            ...
            else if (cardData.energy <= energy)
            {
        ...
        private void ShowUpCard()
        {
            ...
            if (cardActive)
            {
                ...
                GameManager.Instance.EVENT_CARD_SHOWING_UP.Invoke(cardData.id, this.targetPosition);
            }
            else
            {
                auraPS.Stop();
        ...
        private void OnMouseExit()
        {
            ...
            if (cardActive && cardIsShowingUp)
            {
                ...
                GameManager.Instance.EVENT_CARD_MOUSE_EXIT.Invoke(cardData.id);
        ...
        private void OnMouseDrag()
        {
            ...
            //if (type == "attack")
            if (cardData != null && cardData.showPointer)
            {
        ...
        private void OnMouseUp()
        {
            ...
            if (cardData.showPointer)
            {
                GameManager.Instance.EVENT_DEACTIVATE_POINTER.Invoke(cardData.id);
            }
        ...
        private void OnMouseUpAsButton()
        {
            ...
            if (cardActive)
            {
                if (transform.position.y > GameSettings.HAND_CARD_SHOW_UP_Y && card_can_be_played) //if (overPlayer)
                {
                    ...
                    GameManager.Instance.EVENT_CARD_PLAYED.Invoke(cardData.id, "-1");
                    cardActive = false;
                }
                else
                {
        ...
        {
            lastOver = obj;
            if (obj.CompareTag("Player") && card_can_be_played &&
