var EasyIO_WebGL_ImagePicker = {

    // Credit: https://stackoverflow.com/questions/35183253/unity3d-upload-a-image-from-pc-memory-to-webgl-app
    EasyIO_WebGL_GetImage: function(objectNamePtr, funcNamePtr, maxSize) {
      // Because unity is currently bad at JavaScript we can't use standard JavaScript idioms like closures so we have to use global variables
      window.becauseUnitysBadWithJavacript_getImageFromBrowser = window.becauseUnitysBadWithJavacript_getImageFromBrowser || {
         busy: false,
         initialized: false,
      ...
      function handleImageLoad(evt) {
          ...
          // We probably don't want the fullsize image. It might be too big, we can limit the width and height to the given maxSize. 
          var W = img.width;
          var H = img.height;
          var ratio = W / H;
          if (W > maxSize) {
          ...
          g.ctx.canvas.width  = W;
          g.ctx.canvas.height = H;
...
mergeInto(LibraryManager.library, EasyIO_WebGL_ImagePicker);